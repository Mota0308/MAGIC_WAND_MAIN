...
Open up the magic_wand.ino file in the Arduino editor, and make sure the Arduino board is visible and connected to the right port. You'll need to install the following libraries:

1. **From Library Manager** (Sketch → Include Library → Manage Libraries): The [Arduino_LSM9DS1](https://github.com/arduino-libraries/Arduino_LSM9DS1) lets us access the accelerometer and gyroscope readings from the board's IMU (version 1.1.0 or newer). Search for [ArduinoBLE](https://www.arduino.cc/en/Reference/ArduinoBLE) and install version 1.1.3 or newer for Bluetooth.

2. **TensorFlow Lite Micro** (manual install): The TensorFlow Lite library is not in the Library Manager. Download the [tflite-micro-arduino-examples ZIP](https://github.com/tensorflow/tflite-micro-arduino-examples/archive/refs/heads/main.zip) from GitHub. In the Arduino IDE choose **Sketch → Include Library → Add .ZIP Library...** and select the downloaded file. You may need to restart the IDE afterward.
