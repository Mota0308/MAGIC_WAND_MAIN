...
// For Nano 33 BLE Sense *Rev2* only: uncomment the next line if your board list
// has no "Arduino Nano 33 BLE Sense Rev2" (then select "Arduino Nano 33 BLE Sense" as board).
#define ARDUINO_NANO_33_BLE_SENSE_REV2

#include <Wire.h>
// Rev2 uses BMI270+BMM150 (not LSM9DS1). Select "Arduino Nano 33 BLE Sense Rev2" in Tools > Board.
#if defined(ARDUINO_NANO_33_BLE_SENSE_REV2)
#include <Arduino_BMI270_BMM150.h>  // install from Library Manager for Rev2
#else
...
/* TFLITE_SCHEMA_VERSION is defined in micro_interpreter.h */
...
const char* labels[label_count] = {"On","Off"};
...
void setup() {
  Serial.begin(9600);
  // Wait for Serial Monitor to open (Nano 33 BLE Sense USB CDC can be slow)
  ...
  // Explicit I2C init and longer delay before IMU (helps some Nano 33 BLE Sense boards)
  Wire.begin();
  delay(800);

  bool imu_ok = false;
  for (int i = 0; i < 8 && !imu_ok; i++) {
    if (IMU.begin()) {
      imu_ok = true;
      break;
    }
    Serial.println("[LOG] IMU init attempt failed, retrying...");
    delay(400);
  }
  if (!imu_ok) {
    Serial.println("[LOG] Failed to initialize IMU!");
#if defined(ARDUINO_NANO_33_BLE_SENSE_REV2)
    Serial.println("[LOG] Rev2: Install Arduino_BMI270_BMM150 from Library Manager, select board Nano 33 BLE Sense Rev2.");
#else
    // I2C diagnostic: LSM9DS1 accel/gyro at 0x6b WHO_AM_I=0x0f (expect 0x68), mag at 0x1e (expect 0x3d)
    Wire.beginTransmission(0x6b);
    Wire.write(0x0f);
    Wire.endTransmission(false);
    int n_ag = Wire.requestFrom(0x6b, (uint8_t)1);
    uint8_t who_ag = (n_ag == 1) ? Wire.read() : 0xff;
    Wire.beginTransmission(0x1e);
    Wire.write(0x0f);
    Wire.endTransmission(false);
    int n_m = Wire.requestFrom(0x1e, (uint8_t)1);
    uint8_t who_m = (n_m == 1) ? Wire.read() : 0xff;
    Serial.print("[LOG] I2C diag: accel/gyro 0x6b WHO_AM_I=0x");
    Serial.print(who_ag, HEX);
    Serial.print(" (expect 0x68), mag 0x1e WHO_AM_I=0x");
    Serial.print(who_m, HEX);
    Serial.println(" (expect 0x3d). If 0xFF = no response (wrong board or I2C).");
    Serial.println("[LOG] Check: Board=Arduino Nano 33 BLE Sense, Arduino_LSM9DS1 lib, USB cable/port.");
    if (who_ag == 0xff && who_m == 0xff) {
      Serial.println("[LOG] >>> You may have Nano 33 BLE Sense REV2 (BMI270). Uncomment #define ARDUINO_NANO_33_BLE_SENSE_REV2 at top of this file, install Arduino_BMI270_BMM150, then recompile.");
    }
#endif
    while (1);
  }
  ...
  if (!BLE.begin()) {
    Serial.println("[LOG] Failed to initialize BLE!");
    while (1);
  }
  Serial.println("[LOG] BLE OK");
  ...
  Serial.println("[LOG] Loading model...");
  model = tflite::GetModel(g_magic_wand_model_data);
  if (model->version() != TFLITE_SCHEMA_VERSION) {
    Serial.print("[LOG] ERROR: Model schema version ");
  ...
  // New API: (model, op_resolver, tensor_arena, size [, resource_vars, profiler])
  static tflite::MicroInterpreter static_interpreter(
      model, micro_op_resolver, tensor_arena, kTensorArenaSize);
  interpreter = &static_interpreter;

  // Allocate memory from the tensor_arena for the model's tensors.
  Serial.println("[LOG] Allocating tensors...");
  interpreter->AllocateTensors();
  Serial.println("[LOG] Tensors OK");
  ...
  if ((model_input->dims->size != 4) || (model_input->dims->data[0] != 1) ||
    ...
    Serial.println("[LOG] ERROR: Bad input tensor parameters in model");
    TF_LITE_REPORT_ERROR(error_reporter,
  ...
  if ((model_output->dims->size != 2) || (model_output->dims->data[0] != 1) ||
    ...
    Serial.println("[LOG] ERROR: Bad output tensor parameters in model");
    TF_LITE_REPORT_ERROR(error_reporter,
                         "Bad output tensor parameters in model");
    return;
  }

  Serial.println("[LOG] Setup complete. Move the wand to draw a gesture, then stop to see result.");
}

void loop() {
  BLEDevice central = BLE.central();

  // Periodic log so Serial Monitor shows the board is running (every ~5 s)
  static unsigned long last_log_ms = 0;
  if (millis() - last_log_ms >= 5000) {
    last_log_ms = millis();
    Serial.println("[LOG] Waiting for gesture. Move the wand, then hold still to complete.");
  }

  // if a central is connected to the peripheral:
  static bool was_connected_last = false;
  if (central && !was_connected_last) {
    Serial.print("[LOG] Connected to central: ");
  ...
  if (done_just_triggered) {
    ...
    if (invoke_status != kTfLiteOk) {
      Serial.println("[LOG] ERROR: Invoke failed");
